...
1. `golden_prepared` — фиксированный prepared-вход (профиль + события); это ровно `golden_expected` из preprocessing §29.2, поэтому два набора складываются в сквозную проверку raw → token_id.
   Набор обязан фиксировать `golden_vectors_version` препроцессинга и `preprocessing_state_sha256`, при которых снят. Эталон препроцессинга перегенерируется (смена состава `golden_input`, новые краевые случаи), и без записанной версии несовместимость проявится только расхождением `golden_tokens` — то есть будет выглядеть как ошибка токенайзера, хотя изменился вход.
...
6. При загрузке `golden_prepared` записанная `golden_vectors_version` препроцессинга сверяется с версией фактического входа. Несовпадение — блокирующая ошибка: эталон снят с другого входа, и сравнение `golden_tokens` ничего не означает.
7. Golden-vectors и `tokenizer_state_sha256` (§24.1) покрывают разные дыры и не заменяют друг друга — см. оговорку в §24.1.
...
`tokenizer_state_sha256` и golden-vectors (§14.4.3) не заменяют друг друга.

Hash ловит правку конфигурации после BUILD: состояние, пересчитанное при загрузке, расходится с замороженными артефактами. Он **не видит** изменений, состояние не меняющих (правка кодировщика при том же Vocabulary даёт другие `token_id` при том же хэше) и не видит перезаморозки самих артефактов — после нового BUILD состояние снова согласовано с ними.

Golden-vectors ловят и то, и другое: ожидаемые `token_id` и `tokenizer_state_sha256` лежат в эталоне замороженными значениями и помнят прежнее состояние.

Асимметрия между ними — не в том, что каждый ловит, а **когда и на чём**. Golden гоняется на фиксированном маленьком входе перед выпуском: он не увидит того, что проявляется только на данных вне его набора. Hash проверяется при каждой загрузке на любых данных, но выпуск он не блокирует — он блокирует inference. Ни одну из двух проверок нельзя убрать как дублирующую.
